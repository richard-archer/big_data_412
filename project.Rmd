...
#or change the code somehow--columns have percents in them
...
CF_pop<- merge(turnout_cf, resultsm, by="Abbreviation")%>%
  select("Abbreviation",
         "Highest.Office",
         "D",
         "I",
         ...
         "Trump",
         "party_win") %>%
  mutate(vote_pres = gsub( ',', '', CF_pop$Highest.Office),
         vote_per = as.numeric(as.character(vote_pres)),
...
tree_ex<-tree(vote_per~D+R+I, data=turnout_cf)
plot(tree_ex) #this. keeps. giving. me. an error. :(
text(tree_ex)
```
