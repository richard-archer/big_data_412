...
filing <- read.delim("ftf-all-filings.tsv", sep = "\t")
...
ts3 <- merge(ts1, CF_pop, by = "Abbreviation")

#aight ts3 is our master dataframe
```
...
```{r}
#need to resolve missing values in contribution amount 
filing1 <- 
  filing %>%
  select(-c(url,thumbnail_url))
# nb: id is the only column that has no missing or NA values
# id: literally every row is different 
# 
...
#literally find the percentages of columns
andy <- 
  ts3 %>% 
