...
ts2<- merge(ts1, reg_converter, by="Abbreviation")

#at this point, we take our ts dataframe, and we add campaign finance by state
#Campaign finance is from com_agg3, and we sum the total amount spent in that state
State_converter <- read.csv("State_converter.txt")
com_agg3 <- filter(com_agg3, Abbreviation %in% State_converter$Abbreviation)
cf_lasso <- merge(com_agg3, resultsm, by = "Abbreviation")
cf_lasso[is.na(cf_lasso)] <- 0
cf_lasso <- cf_lasso %>% mutate(cf = D + I + R)
#alright at this point, cf_lasso has party spending, total spending, and who won

ts3 <- merge(ts2, cf_lasso, by = "Abbreviation")
```

```{r}
#LASSO TIME BITCHES
#d = campaign finance, dhat = , V = margin or party_win
...
double_lasso <- gamlr(cbind(d,dhat,spm), V, free=2)
